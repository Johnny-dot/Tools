...
function createmd5file($directory) {
    ...
    while(($file = readdir($current_dir)) !== false) {
        $sub_dir = $directory . DIRECTORY_SEPARATOR . $file;
        if($file == "." || $file == "..") {
            continue;
...
function copytoclientpvr ($sub_dir) {
    $fileto = str_replace("../gongce5-25/XA/", "./client_". $GLOBALS["tag"] ."_new/", $sub_dir);
    if (is_dir($sub_dir)) {
...
opreateupdatefile("../gongce5-25/XA/res");
fwrite($updatefile, "}");
echo "*********************************** \n";

echo "****** create all files md5 ******* \n";
$md5file = fopen("./md5_" . $tag .".txt", "w+");
createmd5file("../gongce5-25/XA/res");
echo "*********************************** \n";
