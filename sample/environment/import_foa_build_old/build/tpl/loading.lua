...
local function onSure()
    ...
    print(_sys.networkState, _System.NetNone)
    if _sys.networkState == _System.NetNone then 
        swf.dialogMc._visible = true
        swf.dialogMc.msg.text = '当前网络不可用，请检查您的网络\n是否重试？'
        swf.dialogMc.hitTestDisable = false
    else
        if isUpdate then return end
        swf.dialogMc._visible = false
        swf.dialogMc.hitTestDisable = true
        _sys:httpGet('http://120.132.69.194/foa.php?dev=xxx&c=xxx', function(foaurl)
...
swf.dialogMc._visible = false
swf.dialogMc.hitTestDisable = true
swf.dialogMc.cancelBtn.click = onSure
swf.dialogMc.btnClose.click = onSure
