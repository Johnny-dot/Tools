...
------ 配置
------------------------
local path = 'client_pvr_new/' -- 资源路径
...
local updatelist = _dofile('updatelist_pvr.lua')

local function isoptimizeorcompress( f )
    local flist = string.split(f, "\\")
    local fname = flist[#flist]
    local is = false
    if updatelist[fname] then is = true end
    return is
end
...
local function optimizeSan()
    ...
    local function onEnum(f)
        if f:find('san$') and isoptimizeorcompress(f) then
...
local function optimizeSknAndMsh()
    ...
    local function onEnum(f)
        if (f:find('skn$') or f:find('msh$')) and isoptimizeorcompress(f) then
...
local function toPvr()
    ...
    local genmip = false -- genmip[bool]:是否生成mipmap
    local function onEnum(f)
        if (f:find("tga$") or f:find("bmp$") or f:find("jpg$")) and not f:find(exclude) and isoptimizeorcompress(f) then
