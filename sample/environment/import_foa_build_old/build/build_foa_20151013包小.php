 ...
 * php build_foa_20151013.php platform=android bigversion=v1 isdebug=true tag=dev use_sdk=false use_localserverlist=true lang=zh path=C:\Users\Aoktian\fancy3d\chengxu\trunk\XA sandbox=true toapple=false foc=null schannel=liemo
...
if ($param['platform'] == 'ios') {
    $res = $pdir . ($flag ? '\\res_pvr1' : '\\res_pvr1_new');
    if (!file_exists($res)) {
        $res = $pdir . '\\res';
    }
} elseif ($param['platform'] == 'android') {
    $res = $pdir . ($flag ? '\\res_etc' : '\\res_etc_new');
    if (!file_exists($res)) {
        $res = $pdir . '\\res';
    }
} else {
