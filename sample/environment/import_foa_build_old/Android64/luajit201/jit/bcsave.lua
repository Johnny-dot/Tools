...
local function check(ok, ...)
  ...
  --fancy
--  os.exit(1)
  local fp = io.open( "luajit_ERROR.txt", "w" )
  fp:write("luajit: ", ...)
  fp:write("\n")
...
local function bcsaves(ctx, inputlist)
	local inputs = io.open(inputlist)
	for l in inputs:lines() do
		i = string.find(l, "\t")
		if i == nil or i <= 1 then
			bcsave(ctx, l, l)
		else
			bcsave(ctx, string.sub(l, 1, i-1), string.sub(l, i+1, #l))
		end
	end
end
